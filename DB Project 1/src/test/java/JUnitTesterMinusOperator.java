...
public class JUnitTesterMinusOperator {
	...
	public void testMinus() {
		 ...
		 */
		assertEquals(test_table1, t_minus);
		...
		t_minus = movie.minus(cinema);

		/**
		 * 
		 * movie.minus(cinema) Test 2: When there are no rows inserted in the
		 * movie table. 
		 * Expected Result: no rows in the result table
		 */

		assertEquals(test_table2, t_minus);
		 ...
		 * 
		 * movie.minus(cinema) 
		 * Test 3: When there are few common tuples in the both tables.
		 * Expected Result: R - S i.e. the rows that are only in R  and not in S
		 */
		assertEquals(test_table3, t_minus);
		...
		assertEquals(test_table4, t_minus);
